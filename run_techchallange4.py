...
# ---------------------------------------------
# CONFIGURAÇÕES GLOBAIS E LIMITES AJUSTÁVEIS
# ---------------------------------------------
MIN_LANDMARK_VISIBILITY = 0.1  # Visibilidade mínima para considerar um landmark válido
# FACE_CROP_RATIO_Y e FACE_CROP_RATIO_X foram removidos pois não são mais utilizados.

# Limiares para classificação de postura "deitado"
THRESHOLD_LYING_ASPECT_RATIO = 1.8
THRESHOLD_LYING_VERTICAL_HEIGHT_RATIO = 0.4

# Limiares para postura "sentado"
...
# Limite de deslocamento mínimo para considerar um membro em movimento
MOVEMENT_THRESHOLD_MEMBERS = 5

# Dicionários globais de histórico para 'detect_activities' e 'analyze_member_movement' ---
global_previous_positions_for_activities = {}
global_pose_history_for_member_movement = defaultdict(lambda: [])


# ---------------------------------------------
# FUNÇÃO: Análise emocional com DeepFace
# ---------------------------------------------
def analyze_emotion_deepface(face_image):
    """
    Analisa a emoção dominante em uma imagem facial usando DeepFace.
    Retorna (emoção, score de confiança).
    """
...
# ---------------------------------------------
# FUNÇÃO: Extrai coordenadas (x, y) de um landmark
# ---------------------------------------------
def get_landmark_coords(
    ...
    """
    Retorna a posição (x, y) de um landmark, se ele for visível.
    """
...
# ---------------------------------------------
# FUNÇÃO: Verifica se algum landmark está visível
# ---------------------------------------------
def are_any_landmarks_visible(
    landmarks, landmark_enums, min_visibility=MIN_LANDMARK_VISIBILITY
):
    """
    Verifica se pelo menos um dos landmarks da lista está visível.
    """
    if not landmarks:
        return False
    for enum in landmark_enums:
        if (
            enum.value < len(landmarks)
            and landmarks[enum.value].visibility > min_visibility
        ):
            return True
    return False
...
# ---------------------------------------------
# FUNÇÃO: Ajusta landmarks para o frame original
# ---------------------------------------------
def adjust_landmarks_to_full_frame(
    landmarks, crop_x1, crop_y1, crop_width, crop_height, frame_width, frame_height
):
    """
    Corrige as coordenadas dos landmarks de um recorte para o espaço do frame completo.
    """
...
# ---------------------------------------------
# FUNÇÃO: Cálculo de ângulo entre três pontos
# ---------------------------------------------
def calculate_angle(p1, p2, p3):
    """
    Calcula o ângulo em graus entre três pontos (p1, p2, p3),
    considerando p2 como vértice.
    """
...
# ---------------------------------------------
# FUNÇÃO: Detecção de Posição (mantida)
# ---------------------------------------------
def detect_position(
    landmarks, image_width, image_height, bbox_width, bbox_height, mp_pose_module
):
    """
    Classifica a postura da pessoa como 'Sentado', 'Em Pé', 'Deitado' ou 'Desconhecido'
    com base nas posições dos principais landmarks corporais e nas proporções da caixa delimitadora.

    Parâmetros:
    - landmarks: lista de pontos da pose corporal retornados pelo MediaPipe
    - image_width/image_height: dimensões do frame completo
    - bbox_width/bbox_height: dimensões da caixa da pessoa (YOLO)
    - mp_pose_module: módulo do MediaPipe contendo os enums dos pontos da pose
    """
    if not landmarks:
        return "Desconhecido"

    # Função auxiliar para obter coordenadas dos landmarks visíveis
    get_lm_xy = lambda lm_enum: get_landmark_coords(
        landmarks, lm_enum, image_width, image_height
    )

    # Captura os pontos principais do corpo
    ...
    # Médias verticais para ombro, quadril, joelho e tornozelo (para proporções)
    avg_shoulder_y = (
        np.mean([lm[1] for lm in [left_shoulder, right_shoulder] if lm])
        if any([left_shoulder, right_shoulder])
        else None
    )
    avg_hip_y = (
        np.mean([lm[1] for lm in [left_hip, right_hip] if lm])
        if any([left_hip, right_hip])
        else None
    )
    avg_knee_y = (
        np.mean([lm[1] for lm in [left_knee, right_knee] if lm])
        if any([left_knee, right_knee])
        else None
    )
    avg_ankle_y = (
        np.mean([lm[1] for lm in [left_ankle, right_ankle] if lm])
        if any([left_ankle, right_ankle])
        else None
    )

    # Verifica se há landmarks visíveis na parte inferior do corpo
    lower_body_landmarks_check = [
        ...
        mp_pose_module.PoseLandmark.LEFT_HEEL,
        mp_pose_module.PoseLandmark.RIGHT_HEEL,
        mp_pose_module.PoseLandmark.LEFT_FOOT_INDEX,
        mp_pose_module.PoseLandmark.RIGHT_FOOT_INDEX,
    ]
    is_any_lower_body_lm_visible = are_any_landmarks_visible(
        landmarks, lower_body_landmarks_check
    )

    # Aspect Ratio da caixa (largura / altura)
    aspect_ratio_bbox = bbox_width / bbox_height if bbox_height > 0 else 0

    # Altura vertical estimada da pose
    ...
    # --- DETECÇÃO DEITADO ---
    if (
        aspect_ratio_bbox > THRESHOLD_LYING_ASPECT_RATIO
        and pose_vertical_height < image_height * THRESHOLD_LYING_VERTICAL_HEIGHT_RATIO
    ):
        return "Deitado"
    ...
    if avg_hip_y is not None and avg_knee_y is not None:
        if avg_knee_y > avg_hip_y:
            if avg_ankle_y is not None and avg_ankle_y > avg_knee_y:
                ...
                if (
                    bbox_height > 0
                    and (hip_ankle_dist_y / bbox_height)
                    > STANDING_LEG_TO_BBOX_RATIO_MIN
                ):
                    return "Em Pé"
            else:
                ...
                if (
                    bbox_height > 0
                    and (hip_knee_dist_y / bbox_height)
                    > STANDING_HIP_KNEE_TO_BBOX_RATIO_MIN
                ):
    ...
    is_any_main_upper_body_non_hand_lm_visible = are_any_landmarks_visible(
        landmarks, main_upper_body_non_hand_landmarks
    )

    if is_face_visible or is_any_main_upper_body_non_hand_lm_visible:
        if not is_any_lower_body_lm_visible:
            if (
                bbox_height > image_height * SITTING_BBOX_HEIGHT_MIN_RATIO
                and bbox_height / image_height < 0.6
            ):
                return "Sentado"
            ...
            if (
                bbox_height > 0
                and (knee_hip_diff / bbox_height) < SITTING_KNEE_HIP_MAX_DIST_RATIO
            ):
                return "Sentado"
            ...
            if (
                ang_joelho is not None
                and ANGLE_KNEE_SITTING_MIN <= ang_joelho <= ANGLE_KNEE_SITTING_MAX
            ):
                return "Sentado"

            # Análise de ângulo do quadril (ombro-quadril-joelho)
            ang_quadril_esq = None
            ang_quadril_dir = None
            if left_hip and left_knee:
                if left_shoulder:
                    ang_quadril_esq = calculate_angle(
                        left_shoulder, left_hip, left_knee
                    )
                elif right_shoulder:
                    ang_quadril_esq = calculate_angle(
                        right_shoulder, left_hip, left_knee
                    )

            if right_hip and right_knee:
                if right_shoulder:
                    ang_quadril_dir = calculate_angle(
                        right_shoulder, right_hip, right_knee
                    )
                elif left_shoulder:
                    ang_quadril_dir = calculate_angle(
                        left_shoulder, right_hip, right_knee
                    )
            ...
            if (
                ang_quadril is not None
                and ANGLE_HIP_SITTING_MIN <= ang_quadril <= ANGLE_HIP_SITTING_MAX
            ):
                return "Sentado"
            ...
            if (
                bbox_height > 0
                and (ankle_hip_diff / bbox_height) < SITTING_ANKLE_HIP_MAX_DIST_RATIO
            ):
                return "Sentado"
...
# ---------------------------------------------
# FUNÇÃO: Detecção de Atividades (detect_activities)
# ---------------------------------------------
def detect_activities(
    pose_results_person, person_id, mp_pose_module, image_width, image_height
):
    """
    Detecta atividades específicas de uma pessoa (como levantar a mão, acenar, dançar, aperto de mão)
    com base nos landmarks da pose estimada pelo MediaPipe.

    Parâmetros:
    - pose_results_person: resultado da pose da pessoa retornado pelo MediaPipe
    - person_id: identificador único da pessoa (ID do YOLO tracking)
    - mp_pose_module: módulo do MediaPipe com enums dos pontos da pose
    - image_width, image_height: dimensões do frame
    """
    activities = []
    global global_previous_positions_for_activities

    # Inicializa o histórico para o ID da pessoa, se não existir
    if person_id not in global_previous_positions_for_activities:
        global_previous_positions_for_activities[person_id] = {}

    previous_positions_for_this_person = global_previous_positions_for_activities[
        person_id
    ]

    # Verifica se há landmarks da pose disponíveis
    if pose_results_person and pose_results_person.pose_landmarks:
        landmarks = pose_results_person.pose_landmarks.landmark

        # Função auxiliar para extrair coordenadas visíveis
        get_lm_xy_vis = lambda lm_enum: get_landmark_coords(
            landmarks, lm_enum, image_width, image_height, MIN_LANDMARK_VISIBILITY
        )

        # Captura os principais pontos do corpo usados para detectar as atividades
        ...
        # --- DETECÇÃO: Mão Esquerda Levantada (acima do ombro, sem mover lateralmente)
        if (
            left_hand
            and left_shoulder
            and left_hand[1] < left_shoulder[1]
            and abs(left_hand[0] - left_shoulder[0]) < (0.2 * image_width)
        ):
            activities.append("Mao esquerda levantada")

        # --- DETECÇÃO: Mão Direita Levantada
        if (
            right_hand
            and right_shoulder
            and right_hand[1] < right_shoulder[1]
            and abs(right_hand[0] - right_shoulder[0]) < (0.2 * image_width)
        ):
            activities.append("Mao direita levantada")

        # --- DETECÇÃO: Aceno com a Mão Esquerda (movimento lateral da mão visível)
        if (
            left_hand
            and left_shoulder
            and left_elbow
            and abs(left_hand[1] - left_shoulder[1]) < (0.2 * image_height)
            and abs(left_hand[0] - left_elbow[0]) > (0.3 * image_width)
        ):
            activities.append("Acenando com a mao esquerda")

        # --- DETECÇÃO: Aceno com a Mão Direita
        if (
            right_hand
            and right_shoulder
            and right_elbow
            and abs(right_hand[1] - right_shoulder[1]) < (0.2 * image_height)
            and abs(right_hand[0] - right_elbow[0]) > (0.3 * image_width)
        ):
            activities.append("Acenando com a mao direita")

        # --- DETECÇÃO: Aperto de Mão (mãos próximas e movimento em direção à outra)
        if (
            left_hand
            and right_hand
            and abs(left_hand[0] - right_hand[0]) < (0.2 * image_width)
            and abs(left_hand[1] - right_hand[1]) < (0.2 * image_height)
        ):

            # Recupera posições anteriores
            left_hand_prev = previous_positions_for_this_person.get(
                "left_hand", (None, None)
            )
            right_hand_prev = previous_positions_for_this_person.get(
            ...
            if left_hand_prev[0] is not None and left_hand_prev[1] is not None:
                left_move = np.linalg.norm(
                    np.array(left_hand) - np.array(left_hand_prev)
                )

            right_move = 0
            if right_hand_prev[0] is not None and right_hand_prev[1] is not None:
                right_move = np.linalg.norm(
                    np.array(right_hand) - np.array(right_hand_prev)
                )

            # Se houver movimento entre os frames → é um aperto de mão
        ...
        # --- DETECÇÃO: Dançando (movimentos grandes de vários pontos do corpo)
        ...
        # Compara com posições anteriores
        ...
        # Se a soma dos deslocamentos for grande → dança
        if total_movement > movement_threshold_dancing_pixels:
            activities.append("Dancando")

    # Caso nenhuma atividade tenha sido detectada
    if not activities:
...
# ---------------------------------------------
# FUNÇÃO: Analise de movimento dos membros (analyze_member_movement)
# ---------------------------------------------
def analyze_member_movement(
    ...
    frame_width,
    frame_height,
    movement_threshold=MOVEMENT_THRESHOLD_MEMBERS,
):
    """
    Analisa quais membros do corpo estão se movendo entre dois frames consecutivos.

    Parâmetros:
    - current_pose_landmarks: landmarks da pose no frame atual (MediaPipe)
    - prev_pose_landmarks: landmarks da pose no frame anterior
    - frame_width, frame_height: dimensões do frame (usado para converter coordenadas normalizadas em pixels)
    - movement_threshold: deslocamento mínimo para considerar que houve movimento

    Retorna:
    - Lista de membros com movimento detectado (ex: ['Braco Direito', 'Cabeca'])
    """
    ...
    # Visibilidade mínima para considerar o ponto válido
    visibility_threshold = 0.6

    # Para cada membro do corpo...
    for member, kps in member_landmarks.items():
        total_displacement_for_member = 0 # Soma dos deslocamentos dos pontos
        num_visible_kps_for_member = 0 # Contador de pontos visíveis

        # Para cada ponto-chave (landmark) do membro
        for kp in kps:
            if kp.value < len(landmarks_curr) and kp.value < len(landmarks_prev):
                # Verifica se o ponto é visível nos dois frames
                if (
                    landmarks_curr[kp].visibility > visibility_threshold
                    and landmarks_prev[kp].visibility > visibility_threshold
                ):
                    ...
                    # Calcula distância entre o ponto atual e o anterior
                    distance = np.linalg.norm(
                        np.array([current_x, current_y]) - np.array([prev_x, prev_y])
                    )
                    total_displacement_for_member += distance
                    num_visible_kps_for_member += 1

        # Se pelo menos 1 ponto foi analisado com sucesso
        if num_visible_kps_for_member > 0:
            average_displacement = (
                total_displacement_for_member / num_visible_kps_for_member
            )
            # Se a média do deslocamento for maior que o limiar → considera movimento
...
# ---------------------------------------------
# FUNÇÃO: Função Principal de Processamento de Vídeo
# ---------------------------------------------
def process_video_with_yolo_and_mediapipe_and_deepface(
    input_video_path, output_video_path
):
    """
    Processa um vídeo aplicando:
    - Detecção de pessoas com YOLO
    - Estimativa de pose e mãos com MediaPipe
    - Análise de emoções com DeepFace
    - Detecção de postura e atividades específicas
    - Detecção de movimento por partes do corpo
    - Geração de vídeo anotado e relatório textual final
    """

    # Verifica se o vídeo de entrada existe
    if not os.path.exists(input_video_path):
        print(
            f"Erro: O arquivo de vídeo de entrada não foi encontrado em '{input_video_path}'"
        )
        return
    ...
    pose_estimator = mp_pose_instance.Pose(
    ...
    # Reinicializa os históricos globais
    global global_previous_positions_for_activities
    global global_pose_history_for_member_movement
    global_previous_positions_for_activities = {}
    global_pose_history_for_member_movement = defaultdict(lambda: [])

    # Cria pasta para salvar imagens das pessoas (recortes)
    ...
    while cap.isOpened():
        ...
        # Detecção de pessoas com YOLO (classe 0: pessoa)
        results_yolo = yolo_model.track(
            frame_rgb, persist=True, classes=[0], verbose=False, conf=0.5, iou=0.7
        )

        if results_yolo[0].boxes.id is not None:
            # Extrai as boxes, IDs e confiabilidade
            ...
            for i, box in enumerate(boxes):
                ...
                # Salva imagem da pessoa detectada - Apenas salve se o recorte for válido
                if person_crop_rgb.size > 0:
                    # Converte de volta para BGR antes de salvar, pois o OpenCV espera BGR para salvar
                    cv2.imwrite(
                        os.path.join(
                            output_image_dir,
                            f"{frame_count:04d}_person_{person_id}.jpg",
                        ),
                ...
                if current_crop_width > 0 and current_crop_height > 0:
                    ...
                    if (
                        ...
                        and results_mediapipe_person.pose_landmarks
                    ):
                        ...
                        # Salva histórico da pose
                        global_pose_history_for_member_movement[person_id].append(
                            adjusted_landmarks_for_logic
                        )
                        if (
                            len(global_pose_history_for_member_movement[person_id])
                            > fps
                        ):
                            global_pose_history_for_member_movement[person_id].pop(0)

                    # POSTURA: "Em Pé", "Sentado", "Deitado"...
                    ...
                    # ATIVIDADES: Mão levantada, aceno, dança...
                    ...
                    # MOVIMENTO DE MEMBROS: braços, pernas, cabeça...
                    moving_members_this_person = []
                    if (
                        person_id in global_pose_history_for_member_movement
                        and len(global_pose_history_for_member_movement[person_id]) >= 2
                    ):

                        current_person_pose_lms = (
                            global_pose_history_for_member_movement[person_id][-1]
                        )
                        prev_person_pose_lms = global_pose_history_for_member_movement[
                            person_id
                        ][-2]
                    ...
                    # EMOÇÃO FACIAL
                    dominant_emotion, emotion_score = analyze_emotion_deepface(
                        person_crop_rgb
                    )
                    ...
                    current_text_y = (
                        ...
                        + (
                            text_height
                            if "text_height" in locals()
                            else line_height_increment
                        )
                    )  # Ajuste inicial
                    ...
                    for idx, label_text in enumerate(labels_to_draw):
                        ...
                        if (
                            bg_x2 < x2 and bg_y2 < y2 + 5
                        ):  # Permite um pequeno "vazamento" na parte inferior para evitar corte prematuro
                            ...
                            white_rect = (
                                np.ones(sub_img.shape, dtype=np.uint8) * 0
                            )  # Cor preta

                            res = cv2.addWeighted(
                                sub_img, 0.6, white_rect, 0.4, 1.0
                            )  # Ajuste 0.6 e 0.4 para mais/menos transparência
                            frame[bg_y1:bg_y2, bg_x1:bg_x2] = res

                            cv2.putText(
                                frame,
                                label_text,
                                (x1 + padding_x, current_text_y),
                                font,
                                font_scale,
                                color,
                                font_thickness,
                    ...
                    if (
                        results_mediapipe_hands
                        and results_mediapipe_hands.multi_hand_landmarks
                    ):
                        for (
                            hand_landmarks
                        ) in results_mediapipe_hands.multi_hand_landmarks:
    ...
    with open(report_filename, "w", encoding="utf-8") as report_file:
        ...
        for emotion, count in sorted(overall_emotion_counts.items()):
            ...
            report_file.write(
                f"- {emotion.capitalize()}: {count} frames ({percentage:.2f}%)\n"
            )
        ...
        for member, count in sorted(overall_member_movement_counts.items()):
            percentage = (
                (count / total_member_movements) * 100
                if total_member_movements > 0
                else 0
            )
